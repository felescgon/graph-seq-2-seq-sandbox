...
autoencoder_rama_3 = Autoencoder(X_rama_3.shape[1], 32)
criterion = nn.MSELoss()
optimizer = optim.Adam(autoencoder_rama_3.parameters(), lr=0.001)
...
for epoch in range(num_epochs):
    autoencoder_rama_3.train()
    epoch_train_loss = 0
    for i in range(num_batches):
        batch = X_rama_3_tensor[i * batch_size:(i + 1) * batch_size]
        optimizer.zero_grad()
        outputs, _ = autoencoder_rama_3(batch)  # Forward pass
    ...
    # Calculate validation loss (using the same data for simplicity)
    autoencoder_rama_3.eval()
    with torch.no_grad():
        val_outputs, _ = autoencoder_rama_3(X_rama_3_tensor)  # Forward pass on validation data
...
with torch.no_grad():
    X_rama_3_encoded, _ = autoencoder_rama_3(X_rama_3_tensor)
...
# Define the main model with LSTM
class LSTMModel(nn.Module):
    def __init__(self, input_dim_rama_1, input_dim_rama_2, input_dim_rama_3, output_size):
        ...
        self.lstm = nn.LSTM(1024 * 3, 512, batch_first=True)
        self.output_layer = nn.Linear(512, output_size)

    def forward(self, rama_1, rama_2, rama_3):
        dense_out_1 = self.dense_rama_1(rama_1).unsqueeze(1)  # Adding a sequence dimension
        dense_out_2 = self.dense_rama_2(rama_2).unsqueeze(1)  # Adding a sequence dimension
        dense_out_3 = self.dense_rama_3(rama_3).unsqueeze(1)  # Adding a sequence dimension
        
        combined = torch.cat([dense_out_1, dense_out_2, dense_out_3], dim=1)
        lstm_out, _ = self.lstm(combined)
        
        return self.output_layer(lstm_out[:, -1, :])  # Last time step
...
# Inverse transformation and save predictions
...
predictions_categorical_original = autoencoder_rama_3.decoder(torch.FloatTensor(predictions_categorical.detach().numpy())).detach().numpy()
...
predictions_df = pd.DataFrame(predictions_original, columns=original_df.columns[:-1])
